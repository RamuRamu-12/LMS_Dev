...
import { FiSave, FiX, FiCalendar, FiUsers, FiAward, FiUpload } from 'react-icons/fi';
...
const CreateHackathonPage = () => {
  ...
  const getAuthToken = () => {
    ...
    // Try localStorage with correct key 'accessToken'
    let localToken = localStorage.getItem('accessToken');
    if (localToken) {
      console.log('Using token from localStorage (accessToken)');
      return localToken;
    }
    
    // Try localStorage with 'token' key (fallback)
    ...
    if (sessionToken) {
      console.log('Using token from sessionStorage (accessToken)');
      localStorage.setItem('accessToken', sessionToken); // Sync to localStorage
      return sessionToken;
    }
    ...
    if (sessionToken) {
      ...
      localStorage.setItem('accessToken', sessionToken); // Sync to localStorage with correct key
      return sessionToken;
    }
  ...
  const fetchStudents = async () => {
    try {
      ...
      console.log('Fetching students with token:', authToken.substring(0, 20) + '...');
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-600 mt-2">Set up a new hackathon for students to participate in</p>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="space-y-8" noValidate>
          ...
          >
            <h2 className="text-xl font-semibold text-gray-900 mb-6 flex items-center">
              <FiAward className="w-6 h-6 mr-2 text-indigo-600" />
              Basic Information
            </h2>
          ...
          >
            ...
            <button
              ...
              className="bg-indigo-600 text-white px-6 py-3 rounded-lg hover:bg-indigo-700 transition-colors duration-200 flex items-center space-x-2 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loading ? (
